...
public class UIManager : Singleton<UIManager>
{
    ...
    private UIManager()
    {
        ...
        uiCanvas = GameObject.Instantiate
            (ResourcesResMgr.Instance.LoadAsset<GameObject>("UI/Canvas"))
            .GetComponent<Canvas>();
        ...
        uiEventSystem = GameObject.Instantiate
            (ResourcesResMgr.Instance.LoadAsset<GameObject>("UI/EventSystem"))
    ...
    public async UniTask<T> ShowPanel<T>(E_UILayer layer = E_UILayer.Middle) where T : BasePanel
    {
        ...
        {
            CmgmLog.fNegative($"<color=#b5a642>UI面板加载的UniTask被取消</color>：{ex}");
            return null;
        }
        ...
        if (panelDic.ContainsKey(panelName))
        {
            ...
            if (panelInfo.panel == null) //还在加载，字典里只是空的占位
            {
                if (isDestroy)
                    ...
                    panelInfo.isActive = false; //标记为“等加载完再失活”状态
            }
            else //字典里已经有它的实例了
            {
    ...
    public BasePanel GetTopDynamicPanel()
    {
        ...
        if (topPanel == null)
        {
            CmgmLog.fNegative("没有找到激活且动态的顶层面板，请检查是否正确设置面板的动态性");
        }
