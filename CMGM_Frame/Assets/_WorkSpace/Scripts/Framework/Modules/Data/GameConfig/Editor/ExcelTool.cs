...
public class ExcelTool
{
    ...
    /// excel文件存放的路径
    /// </summary>
    public static string EXCEL_PATH = Consts.Paths.WorkSpace + "/Excels";
    /// <summary>
    /// 数据容器脚本存储位置路径
    /// </summary>
    ...
    /// 表格支持的合法变量类型
    /// </summary>
    public static HashSet<string> VALID_TYPES = new HashSet<string>
    {
                ...
                for (int j = 0; j < table.Columns.Count; j++)
                {
                    ...
                    if (!VALID_TYPES.Contains(typeRow[j].ToString()))
                    {
                        if (typeRow[j] != DBNull.Value)//如果是空白直接删，不是空白报个错再删
                            CmgmLog.fWarning($"在Excel表中出现了不支持的变量类型：|{typeRow[j]}|，序列化时已删除该列");
                        table.Columns.RemoveAt(j);
                        j--; //因为移除了当前列，所以要再检查一下当前列
                    }

                    //2、如果某一格子的主键是空的，直接移除这一行（避免某行为空时进行无提示性报错）
                    if (j == keyIdx)
                    {   //只遍历主键这一列的行
    ...
    [MenuItem("草木句萌/清除并构建Excel数据", false, 112)]
    private static void ClearExcelData()
    {
        if (Directory.Exists(Consts.Paths.ConfigData))
        {
            var files = Directory.GetFiles(Consts.Paths.ConfigData);
